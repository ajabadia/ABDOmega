    ...
    public:
        ...
        void process(float* left, float* right, int numSamples, const Params& p) noexcept {
            for (int i = 0; i < numSamples; ++i) {
                process(left[i], right[i], p);
            }
        }

        inline void process(float& left, float& right, const Params& p) noexcept {
            float inL = left;
            float inR = right;
            
            // [Modular Evolution]: Basic dry/wet mapping for Space Echo
            // Real delay logic is handled by internal JUCE DelayLine
            left = inL * (1.0f - p.echoVol) + (inL * p.echoVol);
            right = inR * (1.0f - p.echoVol) + (inR * p.echoVol);
        }
