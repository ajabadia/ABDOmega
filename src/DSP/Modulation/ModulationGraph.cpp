...
namespace Omega::DSP::Modulation {
    ...
    CompileResult ModulationGraph::compile() {
        ...
        for (const auto& conn : mConnections) {
            // [Innovation]: El nodo Delay1 rompe de forma implícita el ciclo al procesar el valor del bloque anterior.
            // Para el algoritmo de Kahn, no contamos el in-degree de la entrada de un Delay1.
            auto it = std::find_if(mNodes.begin(), mNodes.end(), [&](const auto& n) { return n->id == conn.destNode; });
            if (it != mNodes.end() && (*it)->type != NodeType::Delay1) {
                adj[conn.sourceNode].push_back(conn.destNode);
                inDegree[conn.destNode]++;
            } else if (it != mNodes.end() && (*it)->type == NodeType::Delay1) {
                // El Delay1 se conecta al grafo pero no bloquea el orden de ejecución (es una raíz diferida).
                adj[conn.sourceNode].push_back(conn.destNode);
            }
        }
