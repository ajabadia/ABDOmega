...
namespace Omega::DSP::Modulation {
    ...
    void ModulationRuntime::processNode(RuntimeNode& node, int numSamples) {
        ...
        switch (node.type) {
            ...
            case 10: // Delay1 (Z-1 Feedback)
            {
                // [Hardening]: El valor del paso anterior se lee antes de sobreescribir con el nuevo
                float inputVal = (node.inputIndices[0] != 255) ? mBuffers.values[node.inputIndices[0]] : 0.0f;
                mBuffers.values[node.outputIndex] = node.state.raw[0]; // Salida diferida (Z-1)
                node.state.raw[0] = inputVal; // Almacén para el próximo bloque
                break;
            }
            default:
                break;
