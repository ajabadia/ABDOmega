...
export class ModulePatchbayMatrix {
    ...
    constructor(options: any = {}) {
        ...
        if ((window as any).runtimeStore) {
            (window as any).runtimeStore.subscribe((type: any) => {
                ...
                if (type & 1 /* Structure */) {
                    this.onStateUpdate((window as any).runtimeStore.getSnapshot());
                }
            });
        }
    }
    ...
    private buildMetadataFromInventory(components: any[]) {
        ...
        components.forEach(comp => {
            ...
            if (!activeTypes.has(comp.id)) return;

            if (!comp.registry) return;
            comp.registry.forEach((reg: any) => {
                ...
                if (reg.roles?.includes('output')) newSources.push(item);
                if (reg.roles?.includes('input')) newTargets.push(item);
            });
        });
    ...
    public onStateUpdate(state: any) {
        ...
        const structuralChange = oldModules.length !== newModules.length || 
                                 JSON.stringify(oldModules.map((m: any) => m.id)) !== JSON.stringify(newModules.map((m: any) => m.id));
        ...
        const activeCount = matrix.filter((s: any) => s.active === true || s.active === "true").length;
    ...
    private renderStructure(grid: HTMLElement) {
        let html = '';
        ...
        if (this.viewMode === 'compose') {
            const activeSlots = matrix.map((s: any, i: number) => ({...s, i}))
                .filter((s: any) => (s.active === true || s.active === "true") || (s.source !== '' && s.source !== undefined));
    ...
    private getSlotSkeleton(i: number): string {
        return `
    ...
    private syncSlotsFromState(matrix: any[]) {
        // Delta Update Engine
        matrix.forEach((slot, i) => {
            ...
            if (sourceLabel) sourceLabel.textContent = this.getNameForId(this.sources, slot.source) || 'EMPTY';
            if (targetLabel) targetLabel.textContent = this.getNameForId(this.targets, slot.target) || '---';
    ...
    private addModulation() {
        ...
        let targetSlot = matrix.findIndex((s: any, idx: number) => 
            idx < this.maxSlots && !s.active && !s.source
        );
    ...
    private generateOptions(list: any[], current: string, exclude: string) {
    ...
    private attachGridListeners(grid: HTMLElement) {
        grid.querySelectorAll('.matrix-card').forEach(card => {
            card.addEventListener('click', () => {
                this.selectedSlot = parseInt((card as HTMLElement).dataset.index || "0");
                this.renderWorkspace();
            });
            ...
            if (slider) {
                ...
                const update = (e: MouseEvent) => {
                    ...
                    this.sendUpdate(parseInt((card as HTMLElement).dataset.index || "0"), 'amount', val);
                };
                ...
                slider.addEventListener('pointermove', (e: Event) => { if (isDragging) update(e as MouseEvent); });
                slider.addEventListener('pointerup', () => isDragging = false);
            }
        });
    }

    private attachInspectorListeners(container: HTMLElement) {
        container.querySelectorAll('.inspector-select, .inspector-range').forEach(ctrl => {
            ctrl.addEventListener(ctrl.tagName === 'SELECT' ? 'change' : 'input', (e: any) => {
