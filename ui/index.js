...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (menuPreferences && preferencesModal) {
        menuPreferences.onclick = () => {
            preferencesModal.style.display = 'flex';
            if (window.Preferences) window.Preferences.init();
        };
    }
    ...
    try {
        ...
        const pollTelemetry = async () => {
            ...
            try {
                ...
                // [Modular Evolution]: Add MIDI virtual signal if viewer is active
                if (hasMidi) indices.add(64);
