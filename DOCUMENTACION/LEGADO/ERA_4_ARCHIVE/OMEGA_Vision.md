...
# OMEGA: Visión General del Proyecto (Era 4.1.0 FINAL)

## 👁️ Arquitectura de Doble Capa (Dual-Tier UX)
OMEGA separa la experiencia de usuario en dos niveles lógicos para maximizar la ergonomía y claridad:

### 1. Performance Tier (Front Panel)
- **Propósito**: Interpretación musical y ruteo creativo en tiempo real.
- **Controles**: Perillas (Knobs), Deslizadores (Sliders), Jacks (Ports) y LEDs de estado.
- **Ubicación**: Rack principal (Frontal del módulo).

### 2. Configuration Tier (Back Panel / General)
- **Propósito**: Diseño técnico, calibración y selección de modos de comportamiento.
- **Controles**: Menús desplegables (Lists), entradas numéricas de precisión, interruptores de modo.
- **Ubicación**: Pestaña "General" dentro de la modal de configuración del módulo (Panel Trasero).
...
## 3. Telemetría Aséptica y Roteo de Señal
OMEGA no utiliza índices de señal hardcodeados hacia la interfaz. La telemetría sigue un modelo de **Dos Velocidades** basado en el descubrimiento dinámico.

### 3.1. Modelo de Dos Velocidades (Sampling Strategy)
*   **Velocidad Alta (Audio/Modulation Stream)**:
    *   Destinado al **Osciloscopio / Wave Analyzer**.
    *   Envía buffers completos (ej: 1024 muestras) para representar formas de onda reales.
    *   Solo se activa bajo demanda cuando la herramienta visual correspondiente está abierta.
*   **Velocidad Baja (Discrete/Discrete Logic)**:
    *   Destinado a **LEDs, Vúmetros y Medidores de CPU**.
    *   Utiliza una lógica de **Peak-Hold**: el motor DSP captura el valor máximo en un intervalo (ej: 50ms) para asegurar que pulsos rápidos sean visibles.
    *   Optimiza el tráfico RPC en un 99% en comparación con el streaming constante.

### 3.2. El "Social Contract of Pins"
Cada módulo registra sus propios **Tele-Pins** al inicializarse. Se utilizan identificadores semánticos estándar para permitir que la UI decida automáticamente cómo renderizarlos:
*   `activity`: Parpadeo de LED principal (normalmente verde/azul). **Obligatorio** para todo módulo con lógica.
*   `signal_main`: Fuente primaria para el osciloscopio.
*   `cpu_load`: Utilizado por el monitor de rendimiento del sistema.

### 3.3. Direccionamiento Asepsis
La UI se suscribe a señales usando la ruta: `instancia:pin` (ej: `midi_in_1:activity`). Esto garantiza que el motor sea agnóstico respecto a qué módulo está publicando la información.

## 🕒 Asepsia Temporal (Dynamic Timebase)
OMEGA prohíbe terminantemente el uso de constantes de frecuencia de muestreo hardcodeadas.
- **Dynamic Adaptability**: Prohibido usar `44100`, `48000` o similares en cálculos DSP.
- **Host-Driven**: Todo cálculo debe utilizar el `sampleRate` proporcionado dinámicamente por JUCE o el DAW.
- **WASM Independence**: Los módulos externos deben recalcular sus coeficientes en cada llamada a `omega_init` o ante cambios de configuración del Host.

## 💾 Persistencia Profunda (Deep Persistence)
La integridad del sonido depende de que cada instancia de módulo guarde su estado total.
- **State Capture**: Los patches de OMEGA no solo guardan el cableado (ruteo), sino el valor exacto de cada parámetro definido en el manifiesto ACE.
- **Agnosticismo de Visibilidad**: No importa si un parámetro es frontal o trasero (Back Panel); su persistencia es obligatoria y universal.
- **Serialización ACE**: Todas las propiedades del módulo en el preset deben mapearse al estado del motor WASM en la recarga.

## 🚦 Visibilidad Inteligente (Smart Visibility)
El sistema decide automáticamente la ubicación de un parámetro basándose en su semántica:
- **Default Back**: Parámetros de tipo `list` se asignan al **Configuration Tier** para evitar saturación visual en el rack.
- **Default Front**: Controles continuos y puertos se asignan al **Performance Tier**.
- **ACE Override**: El metadato `visibility` en el YAML puede forzar la presencia en `["front", "back", "both"]`.

### 6.4 Unificación Semántica (Smart Bridge)
Para garantizar una UX coherente, la UI de OMEGA implementa un descubrimiento unificado:
1.  **Agnosticismo de Origen**: No importa si el motor reporta un control como `parameter` o como `port` (puerto modulable).
2.  **Clasificación por Tipo**: Si un elemento tiene semántica de configuración (`list`, `number`, `text`) o contiene `options`, la UI lo tratará como un control del **Configuration Tier (Back)**.
3.  **Higiene Visual**: Todo elemento que sea ascendido a la pestaña de configuración será automáticamente filtrado de la pestaña de conectividad (Patching), manteniendo la asepsia de los racks.

---

## 4. El Patchbay / Mod Matrix 2.0
...
### 11.1 Tabla de Código de Colores Oficial (v4.1)
| Color | Tipo de Dato | Aplicación Común |
| :--- | :--- | :--- |
| **Azul** | `voltage` (DSP) | Audio, LFOs, Modulación de Señal |
| **Amarillo** | `midi` | Teclados, Controladores, UMP |
| **Verde** | `list` | Menús de selección, Categorías |
| **Cian** | `float` (GUI) | Posición de Knobs, Sliders, Parámetros UI |
| **Rojo** | `text` | Etiquetas, Nombres de Presets, Metadatos |
...
### 9. Persistencia Profunda (Deep Persistence)
- El guardado de patches es **exhaustivo**: debe incluir tanto el ruteo del Patchbay como el estado interno de todos los parámetros de cada instancia.
- La invisibilidad en el frontal no exime a un parámetro de su persistencia.
