...
namespace Omega::DSP::Engines::Juno {
    ...
    public:
        ...
        float process(int voiceIndex) noexcept {
            ...
            // Curva de detune simplificada (en el futuro usar curva de Szabo)
            // Central osc at index 0, others detuned around it
            const float detuneAmts[kNumOscillators] = { 0.0f, -0.05f, 0.05f, -0.1f, 0.1f, -0.15f, 0.15f };

            for (int i = 0; i < kNumOscillators; ++i) {
                // Generación de sierra básica (0 a 1)
                float saw = (float)(2.0 * v.phases[i] - 1.0);
                
                // PolyBLEP simplificado (opcional para el MVP, pero se recomienda)
                // saw -= polyBlep(v.phases[i], phaseStep);

                sum += saw;

                // Actualizar fase con detune
