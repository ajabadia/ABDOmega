...
set "LOCAL_LLVM=D:\desarrollos\ABDOmega\clang+llvm-18.1.8-x86_64-pc-windows-msvc\bin\clang.exe"
if exist "!LOCAL_LLVM!" (
    set "CLANG_CMD=!LOCAL_LLVM!"
    echo [INFO] Using Local LLVM 18.1.8: !CLANG_CMD!
) else (
...
:: 2. Compile each .c file
for %%f in ("%PLUGIN_SRC%\*.c") do (
    set "FILENAME=%%~nf"
