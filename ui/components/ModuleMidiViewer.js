...
export class ModuleMidiViewer {
    ...
    updateSourceSelector() {
        ...
        for (const s of this.sources) {
            if (s.telemetryIndex === 64)
                continue; // Skip redundant global
    ...
    sources = [];
    selectedSource = 64; // Default MIDI_TRAFFIC
    render() {
    ...
    bindEvents() {
        ...
        if (pwrBtn) {
            pwrBtn.onclick = () => {
                ...
                if (!this.isPowered)
                    this.addLogLine({ ts: Date.now() / 1000, type: 0, ch: 0, d1: 0, d2: 0 }, "MONITOR PAUSED");
            };
        }
        if (sel) {
            sel.onchange = () => {
...
//# sourceMappingURL=ModuleMidiViewer.js.map